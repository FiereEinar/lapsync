...
export function EditEventModal({ event, visible, onClose, onSuccess }: { event: any, visible: boolean, onClose: () => void, onSuccess: () => void }) {
  ...
  useEffect(() => {
    if (event && visible) {
       setForm({
         ...
         venue: event.location?.venue || (typeof event.location === 'string' ? event.location : ''),
       ...
       if (event.raceCategories && event.raceCategories.length > 0) {
          setCategories(event.raceCategories.map((c:any) => ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert("Error", e.response?.data?.message || "Failed to update event");
    } finally {
      setLoading(false);
    }
  };
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} className="flex-1 bg-background">
        ...
        <ScrollView contentContainerStyle={{ padding: 24 }}>
           ...
           <Button className="mt-6 mb-10 w-full h-14" disabled={loading} onPress={handleSubmit}>
              <Text className="text-primary-foreground font-bold text-lg">{loading ? "Saving..." : "Save Changes"}</Text>
           </Button>
        </ScrollView>
