...
export function MapTrack({ event }: { event: any }) {
  const [activeTab, setActiveTab] = useState<"replay" | "live" | "checkpoints">(
    "live",
  );
  ...
  const handleExport = async () => {
    Alert.alert(
      "Export Telemetry",
      "CSV Export mapping natively requires file-system integration. Would you like to trigger an email export?",
      [
        { text: "Cancel", style: "cancel" },
        {
          text: "Send",
          onPress: () => Alert.alert("Success", "Export requested internally."),
        },
      ],
    );
  };
    ...
    <View className='mb-12'>
      <View className='bg-card items-center p-3 rounded-xl border border-border/60 mb-4 gap-4 z-10 sm:w-full'>
        <View className='flex-row w-full bg-muted/40 p-1.5 rounded-xl'>
          ...
          >
            <Play
              size={14}
              color={
                activeTab === "replay" ? "hsl(0, 0%, 20%)" : "hsl(0, 0%, 50%)"
              }
              style={{ marginRight: 6 }}
            />
            <Text
              className={`text-sm font-bold ${activeTab === "replay" ? "text-foreground" : "text-muted-foreground"}`}
            >
              Replay
            </Text>
          </TouchableOpacity>
          ...
          >
            <Radio
              size={14}
              color={
                activeTab === "live" ? "hsl(173, 50%, 50%)" : "hsl(0, 0%, 50%)"
              }
              style={{ marginRight: 6 }}
            />
            <Text
          ...
          >
            ...
            />
            <Text
              className={`text-sm font-bold ${activeTab === "checkpoints" ? "text-foreground" : "text-muted-foreground"}`}
            >
              Checkpoints
            </Text>
          </TouchableOpacity>
        ...
        <TouchableOpacity
          ...
          className='flex-row items-center justify-center w-full bg-transparent border border-border/60 py-3 rounded-xl'
        >
          <Download
            size={16}
            color='hsl(0, 0%, 40%)'
