              ...
              <View className='flex flex-col gap-3'>
                ...
                {latestEvents.map((event: any, index: number) => {
                  ...
                  if (event.raceCategories) {
                    participants = event.raceCategories.reduce(
                      (acc: number, cur: any) =>
                        acc + (cur.registeredCount || 0),
                      0,
                    );
                  }
                  const displayLocation =
                    typeof event.location === "object"
                      ? `${event.location?.venue || ""} ${event.location?.city || ""}`.trim()
                      : event.location;
                  return (
                    <View
                      key={event._id || index}
                      className='flex-row items-center p-4 bg-muted/30 rounded-xl overflow-hidden'
                    >
                      {/* Date badge */}
                      <View
                      ...
                      <View className='flex-1 pr-3'>
                        <Text
                          className='text-foreground font-bold text-base mb-1.5'
                          numberOfLines={1}
                        >
                          {event.name}
                        </Text>
                      ...
                      <View className='items-center'>
                        ...
                        <Text className='text-muted-foreground text-[9px] uppercase tracking-widest font-bold'>
                          Runners
                        </Text>
                      </View>
                    </View>
          ...
          <Card className='mb-4'>
            ...
            <CardContent className='flex flex-col gap-3'>
              ...
              >
                ...
                <Text className='text-foreground font-semibold ml-2.5 text-sm'>
                  Manage Participants
                </Text>
              </TouchableOpacity>
            </CardContent>
