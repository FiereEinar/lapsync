...
import { PersonalReport } from "@/src/components/tabs/event-detail/PersonalReport";
...
export default function ClientEventDetails() {
  ...
  const finishedTabs = [
    ...
    { id: "reports", label: "My Report", icon: Activity },
  ];

  return (
    <ScrollView className='flex-1 bg-background' stickyHeaderIndices={isFinished ? [2] : undefined}>
      ...
      {isFinished ? (
        <>
          ...
          <View className='bg-background pb-3 pt-4 border-b border-border/50 z-10'>
            <ScrollView
              horizontal
              showsHorizontalScrollIndicator={false}
              contentContainerStyle={{ gap: 8, paddingHorizontal: 24 }}
            >
              {finishedTabs.map((tab) => {
                const Icon = tab.icon;
